...
public class ImportDeviceIOSettinsWindow extends ApplicationWindow {
	...
	private static final Logger log = Logger
			.getLogger(ImportDeviceIOSettinsWindow.class);
	...
	public ImportDeviceIOSettinsWindow(Shell parentShell,
...
//		};
		...
		save.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if(hasEdit) {
					...
					MessageDialog.openInformation(getShell(), "提示", "导入的设备IO信息未做任何修改！");
					return;
				}
			}
		});
		...
		help.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				new ImportDeviceIOHelp(getShell()).open();
			}
		});
	...
	private void save() {
		if (allVariables != null && !allVariables.isEmpty()) {
			...
			for (TagModel tagModel : allVariables) {
				...
				String xpath = GetXpathUtil
						.getTagModelMainIndexXPath(tagModel
								.getMainIndex());
				Element indexNodeElement = (Element) document
						.selectSingleNode(xpath);
				IndexNodeModel indexNode = new IndexNodeModel(
						indexNodeElement);
				...
				// 更新变量名字
				String varType = tagModel.getElement().attributeValue(
						"type");
				String varName = tagModel.getElement().attributeValue(
						TagModel.VAR_NAME_ATTR);
				String oldName = varName.substring(0,
						varName.length() - 4);
				String newNum = GetVarNum.instanse.getVarNum(varType);
				String newName = oldName + newNum;
				tagModel.getElement().addAttribute(
						TagModel.VAR_NAME_ATTR, newName);

				if(!tagDao.addVariable(indexNode, tagModel)) {
					total --;	//导入不成功则减1
				}

			}
			XMLDocumentFactory.instance.saveMainDocument();
			MessageDialog.openInformation(getShell(), "信息", "成功导入" + total + "个变量");
		}
	}
	...
	public boolean close() {
		if(hasEdit) {
			MessageDialog dialog = new MessageDialog(getShell(), "提示", null, "导入的设备IO信息已更改，是否保存？",
