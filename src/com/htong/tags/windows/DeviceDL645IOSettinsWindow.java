...
public class DeviceDL645IOSettinsWindow extends ApplicationWindow {
	...
	private static final Logger log = Logger
			.getLogger(DeviceDL645IOSettinsWindow.class);
	...
	private class ViewerLabelProvider extends LabelProvider implements
		...
		public Image getColumnImage(Object element, int columnIndex) {
			...
			Image image = ResourceManager.getPluginImage("htconfig-slyt", "icons/unedit.gif");
			switch(columnIndex) {
			case 6://偏移量
...
//				}
		...
		public String getColumnText(Object element, int columnIndex) {
			...
			case 12:// 主动上传
				return v.getIoInfo().getPushData() == null ? "" : v.getIoInfo()
						.getPushData();
	...
	public DeviceDL645IOSettinsWindow(Shell parentShell, DeviceModel device, ChannelModel channel) {
		super(parentShell);
	...
	protected Control createContents(Composite parent) {
		...
		gridViewerColumn_3
				.setEditingSupport(new EditingSupport(gridTableViewer) {
					...
					protected void setValue(Object element, Object value) {
						...
						int regAddr = 0;
						try {
							regAddr = Integer.parseInt((String) value, 16);
						} catch (NumberFormatException e) {
							if (!((String) value).isEmpty()) {
								MessageDialog.openError(null, "错误",
										"请输入正确的标识编码！");
								...
								if(im.isFanwei()) {//按范围
									...
									for(;start<=end;start ++) {
										...
										ioInfoModel.setRegAddress("".equals(String
												.valueOf(regAddr)) ? null : Integer.toHexString(regAddr).toUpperCase());
		...
		gridViewerColumn_4
				.setEditingSupport(new EditingSupport(gridTableViewer) {
					...
					protected boolean canEdit(Object element) {
//						TagModel tagModel = (TagModel) element;
//						if (tagModel.getType().startsWith("遥信") || tagModel.getType().startsWith("遥脉")) {
//							return true;
//						} else {
//							return false;
//						}
				...
				.setEditingSupport(new EditingSupport(gridTableViewer) {
					...
					protected void setValue(Object element, Object value) {
						...
						try {
							...
							if(valueType<0 || valueType>ModbusValueTypeEnum.values().length) {	//0-15
								MessageDialog.openError(null, "错误",
										"没有该值类型！");
								return;
							}
		...
		gridColumn_12.setWidth(60);
		gridColumn_12.setText("主动上传");
		gridColumn_12.setMoveable(true);
		gridViewerColumn_12.setEditingSupport(new EditingSupport(
				gridTableViewer) {
			private int gridTopIndex = 0;
			...
			protected Object getValue(Object element) {
				...
				TagModel var = (TagModel) element;
				if (var.getIoInfo().getPushData() != null) {
					return var.getIoInfo().getPushData();
				}
				return "";
			}
			...
			protected void setValue(Object element, Object value) {
				...
				int pushData = 0;
				try {
					pushData = Integer.parseInt((String) value);
					if(pushData !=0 && pushData !=1) {
						MessageDialog.openError(null, "错误", "主动上传值只能是1或者0！");
						return;
					}
				} catch (NumberFormatException e) {
					if (!((String) value).isEmpty()) {
						MessageDialog.openError(null, "错误", "请输入正确的值！");
						return;
					}
				}
				...
				if (tags.size() > 1) {
					...
					if (dlg.open() == Window.OK) {
						...
						if(im.isFanwei()) {//按范围
							...
							for(;start<=end;start ++) {
								IOInfoModel ioInfoModel = ((TagModel)gridTableViewer.getElementAt(start-1)).getIoInfo();
								ioInfoModel.setPushData("".equals(String.valueOf(pushData)) ? null
										: String.valueOf(pushData));

								if (im.getType() > 0) {
									pushData += im.getInterval();
								} else if (im.getType() < 0) {
									pushData -= im.getInterval();
								}
							}
					...
					String v = (String) value;
					if (tag.getIoInfo() != null) {
						tag.getIoInfo().setPushData("".equals(v) ? null : v);
						gridTableViewer.update(tag, null);
						grid.setTopIndex(gridTopIndex);
						return;
					}
		...
		gridColumn_12.setCellSelectionEnabled(true);
...
//					if(io.getRegAddress() == null || "".equals(io.getRegAddress())) {
//						continue;
//					}
